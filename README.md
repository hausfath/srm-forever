...
- Discounting follows **Weitzman's certainty-equivalent (gamma) form**: the social discount rate is treated as uncertain with mean μ (default 1%) and standard deviation σ (default 1%), giving discount factor (1 + σ²t/μ)^(−μ²/σ²) — the effective rate declines from μ toward zero at long horizons, because low-rate futures dominate far-future present values ([Weitzman 1998](https://doi.org/10.1006/jeem.1998.1052), [2001](https://www.aeaweb.org/articles?id=10.1257%2Faer.91.1.260)). Setting σ = 0 recovers conventional exponential discounting. The breakeven mean rate is found by bisection at fixed σ and is identical under either accounting. The page closes with an essay on the discount-rate debate (descriptive vs. prescriptive positions, declining-rate schedules, and why the choice is ultimately normative rather than empirical).
