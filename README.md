...
- Discounting follows **Weitzman's certainty-equivalent (gamma) form**: the social discount rate is treated as uncertain with mean μ (default 1%) and standard deviation σ (default 1%), giving discount factor (1 + σ²t/μ)^(−μ²/σ²) — the effective rate declines from μ toward zero at long horizons, because low-rate futures dominate far-future present values ([Weitzman 1998](https://doi.org/10.1006/jeem.1998.1052), [2001](https://www.aeaweb.org/articles?id=10.1257%2Faer.91.1.260)). Setting σ = 0 recovers conventional exponential discounting, and an in-page chart shows the implied rate schedule and per-year weights. The breakeven mean rate is found by bisection at fixed σ and is identical under either accounting. The page closes with an essay on the discount-rate debate (descriptive vs. prescriptive positions, declining-rate schedules, and why the choice is ultimately normative rather than empirical).
...
At the default settings (Weitzman discounting with μ = σ = 1%, $30/t mitigation over a 30-yr financing period, $100/t CDR at 2 GtCO₂/yr, SRM $1.8B per 0.1°C with exponent 1.3), **the SRM scenario is cheaper in NPV** (~$42T vs. ~$55T) — but the TCRE likely range straddles the verdict (ΔNPV −$24T to +$4T), and the breakeven mean discount rate is ~0.9%, squarely inside the range experts actually debate. Under constant exponential discounting (σ = 0) the breakeven falls to ~0.08%; undiscounted, mitigation + CDR wins (~$127T vs. ~$511T). The verdict turns almost entirely on the discounting choice and the ZEC extrapolation — the tool is built to make those dependencies explicit rather than to hide them.
